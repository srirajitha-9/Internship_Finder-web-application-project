...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if(!$name || !$email || !$password || !$category) $msg = "Fill required fields.";
    else {
        $stmt = $conn->prepare("SELECT id FROM companies WHERE email=? LIMIT 1");
        $stmt->bind_param("s",$email); $stmt->execute(); $stmt->store_result();
        if($stmt->num_rows>0) $msg = "Email already registered.";
        else {
            $hash = password_hash($password, PASSWORD_DEFAULT);
            $ins = $conn->prepare("INSERT INTO companies (name,email,password,category) VALUES (?,?,?,?)");
...
<!doctype html>
<html>
<head><meta charset="utf-8"><title>Company Signup</title><link rel="stylesheet" href="style.css"></head>
<body>
  <div class="container">
    <div class="card" style="max-width:700px;margin:auto">
      ...
      <form method="post">
        <div class="form-row"><label>Company Name</label><input type="text" name="name" required></div>
